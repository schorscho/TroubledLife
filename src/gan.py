...
def generator(Z, seq_length, seq_length_max, n_layers, n_neurons, n_outputs, gru, leaky, input_keep_prob):
    with tf.variable_scope("t_generator"):
        ...
        outputs, states = tf.nn.dynamic_rnn(cell=multi_layer_cell, inputs=Z, sequence_length=seq_length, dtype=tf.float64)
...
def generator_loss(size_batch, Dg):
    ...
    zeros = tf.zeros(size_batch, tf.int64)

    loss = tf.nn.sparse_softmax_cross_entropy_with_logits(labels=zeros, logits=Dg)
    loss = tf.reduce_mean(loss)

    accuracy = tf.reduce_mean(tf.cast(tf.nn.in_top_k(Dg, zeros, 1), tf.float64))
...
def discriminator(X, seq_length, n_layers, n_neurons, n_outputs, leaky, reuse=False):
    with tf.variable_scope("t_discriminator"):
        ...
        outputs, states = tf.nn.dynamic_rnn(cell=multi_layer_cell, inputs=X, sequence_length=seq_length, dtype=tf.float64)
...
def discriminator_loss_real(Dx, y):
    ...
    loss = tf.reduce_mean(tf.cast(loss, tf.float64))

    accuracy = tf.reduce_mean(tf.cast(tf.nn.in_top_k(Dx, y, 1), tf.float64))
...
def discriminator_loss_fake(size_batch, Dg):
    ...
    accuracy = tf.reduce_mean(tf.cast(tf.nn.in_top_k(Dg, ones, 1), tf.float64))
